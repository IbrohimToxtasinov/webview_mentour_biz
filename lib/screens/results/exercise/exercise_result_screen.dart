...
import 'package:mentour_web_view/cubits/ai_analysis/ai_analysis_cubit.dart';
          ...
          children: [
            Column(
              children: [
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        ...
                        Text(
                          "+${resultModel.totalScoreEarned} Score",
                          style: TextStyle(
                            fontSize: 18,
                            color: t.mentourText3,
                            fontWeight: FontWeight.w700,
                ...
                SizedBox(height: 10),
                if (widget.type.toUpperCase() == "GRAMMAR" &&
                    resultModel.questions.any((q) => !q.correct))
                  BlocProvider(
                    create: (context) => AiAnalysisCubit(),
                    child: BlocConsumer<AiAnalysisCubit, AiAnalysisState>(
                      listener: (context, state) {
                        if (state.aiAnalysisFormStatus ==
                                FormStatus.getExerciseAiAnalysisResultSuccess &&
                            state.aiAnalysisResultModel.totalQuestions > 0) {
                          final wrongItems = state
                              ...
                              .toList();
                          _showAiErrorAnalysisBottomSheet(
                            context,
                            wrongItems,
                            t,
                          );
                        } else if (state.aiAnalysisFormStatus ==
                            FormStatus.getExerciseAiAnalysisResultFailure) {
                          showOverlayMessage(context, text: state.errorMessage);
                        }
                      },
                      builder: (context, state) {
                        return StadiumGradientButton(
                          height: 50,
                          gradientColors: [
                            t.newMentourPrimary5,
                            t.newMentourPrimary2,
                          ],
                          label: "ai_error_analysis".tr(),
                          ...
                          onTap: () {
                            if (state.aiAnalysisFormStatus ==
                              ...
                              context
                                  ...
                                  .getExerciseResultAiAnalysisByTaskId(
                                    taskId: widget.taskId,
                                  );
                            }
                          },
                          isLoading:
                              state.aiAnalysisFormStatus ==
                              FormStatus.getExerciseAiAnalysisResultLoading,
                        );
                      },
                    ),
                  ),
...
void _showAiErrorAnalysisBottomSheet(
  BuildContext context,
  List<MapEntry<int, Question>> wrongQuestions,
  ThemeData t,
) {
            ...
            children: [
              ...
              Flexible(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return _AiErrorAnalysisQuestionCard(
                      question: wrongQuestions[index].value,
                      originalIndex: wrongQuestions[index].key,
                    );
                  },
                ),
              ),
...
class _AiErrorAnalysisQuestionCard extends StatefulWidget {
  final Question question;
  final int originalIndex;

  const _AiErrorAnalysisQuestionCard({
    required this.question,
    required this.originalIndex,
  });

  @override
  State<_AiErrorAnalysisQuestionCard> createState() =>
      _AiErrorAnalysisQuestionCardState();
}

class _AiErrorAnalysisQuestionCardState
    extends State<_AiErrorAnalysisQuestionCard> {
      ...
      child: Column(
        children: [
          ListTile(
            ...
            title: Text(
              "${"question".tr()} ${widget.originalIndex + 1}",
              style: TextStyle(
                fontWeight: FontWeight.bold,
                color: t.newMentourText3,
              ),
          ...
          AnimatedCrossFade(
            ...
            secondChild: Container(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: t.newMentourPrimary5.withOpacity(0.05),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                    color: t.newMentourPrimary5.withOpacity(0.1),
                  ),
                ),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
